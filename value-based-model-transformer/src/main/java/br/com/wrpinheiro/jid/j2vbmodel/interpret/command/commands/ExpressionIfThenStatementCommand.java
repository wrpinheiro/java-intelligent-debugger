...
import br.com.wrpinheiro.jid.programdiagnosis.programcomponents.patterns.SimpleSelection;
 ...
 * @author wrp
 */
public class ExpressionIfThenStatementCommand implements
  ...
  private static final Logger LOG = Logger
      .getLogger(ExpressionIfThenStatementCommand.class);
  ...
  public void inNode(InterpretationContext context, Node aNode,
    ...
    AExpressionIfThenStatement node = (AExpressionIfThenStatement) aNode;

    LOG.debug("Starting if-then model construction.");

    String name = context.getNextComponentId();
    SimpleSelection c = new SimpleSelection(context.getSystem(), name,
        DefaultSets.toSet(node.getIf().getLine()), ParserUtils.getInfo(aNode));
    ...
    c.setExpressionComponents(newComps);
    ...
    BlockComponent c_else = new ConditionalBranchComponent(c, c.getName()
        + "_else", DefaultSets.toSet(node.getIf().getLine()), ParserUtils
